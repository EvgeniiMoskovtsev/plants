...
def send_data():
    ...
    t, h = read_sensor_data()  # Чтение данных сенсора
    socketio.emit('update_data', {'t': t, 'h': h, 'conditioner_status': conditioner_status, 'manual_control': manual_control})
...
def index():
    ...
    <head>
        ...
        <script>
            ...
            socket.on('update_data', function(data) {
                document.getElementById("time").innerHTML = "Текущее время: " + data.formatted_time;
                document.getElementById("temperature").innerHTML = "Температура: " + data.t + " °C";
                document.getElementById("humidity").innerHTML = "Влажность: " + data.h + " %";
                document.getElementById("conditioner_status").innerHTML = "Состояние кондиционера: " + (data.conditioner_status ? "Включен" : "Выключен");
                document.getElementById("control_mode").innerHTML = "Режим управления: " + (data.manual_control ? "Ручное Управление" : "Автоматическое Управление");
            });
    ...
    <body>
        ...
        <p id="time">Текущее время: {{ formatted_time }}</p>
        <p id="temperature">Температура: {{ t }} °C</p>
        <p id="humidity">Влажность: {{ h }} %</p>
        <p id="conditioner_status">Состояние кондиционера: {{ "Включен" if conditioner_status else "Выключен" }}</p>
        <p id="control_mode">Режим управления: {{ "Ручное Управление" if manual_control else "Автоматическое Управление" }}</p>
